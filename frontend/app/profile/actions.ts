 ...
 * This should be called every 30 seconds or so
 */
export async function sendHeartbeat(
  userId: string
): Promise<ActionResponse<HeartbeatResponse>> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_ENCORE_API_URL || 'https://staging-everydle-6dz0.encr.app'}/user/presence/heartbeat`, {
      method: 'POST',
...
export async function getPresence(
  userIds: string[]
): Promise<ActionResponse<PresenceResponse>> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_ENCORE_API_URL || 'https://staging-everydle-6dz0.encr.app'}/user/presence/status`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
